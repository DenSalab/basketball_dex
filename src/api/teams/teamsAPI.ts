...
export const teamsAPI = {
    getTeams(name: string, page: number, pageSize: number, token?: string): Promise<TeamDtoPageResult> {
        // const uri = encodeURIComponent(JSON.stringify(getTeamsRequest))
        return get(`/api/Team/GetTeams?Name=${name}&Page=${page}&PageSize=${pageSize}`, token)
    },
    getTeam(id: number, token: string): Promise<TeamDto> {
        return get(`/api/Team/Get?id=${id}`, token)
    },
    addTeam(teamData: TeamDto, token?: string): Promise<TeamDto> {
        return post<TeamDto>('/api/Team/Add', teamData, token);
    },
